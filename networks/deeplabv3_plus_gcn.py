...
from networks import gcn, graph
...
class DeepLabv3_plus_gcn(nn.Module):
    def __init__(self, in_channels=1, out_channels=11, os=16, num_vertebra_points=5, hidden_layers=128, gcn_num = 3,
        ...
        super(DeepLabv3_plus_gcn, self).__init__()
        self.in_channels = in_channels
        self.num_vertebra_points = num_vertebra_points
        self.gcn_num = gcn_num
        # Atrous Conv
        self.xception_features = Xception(in_channels, os, pretrained, normalization=normalization, num_groups=num_groups)
        ...
        # adopt [1x1, 24] for channel reduction.
        ...
        self.graph_2_fea = gcn.Graph_to_Featuremaps(hidden_layers=hidden_layers, output_channels=128)


    def forward(self, img):
        input = self._preprocess(img)
        adj = self.get_graph()
        x, low_level_features = self.xception_features(input) # stride = 16 and 4 respectively
        x1 = self.aspp1(x)
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
        x4 = self.aspp4(x)
        ...
        x = torch.cat((x1, x2, x3, x4, x5), dim=1) # stride = 16

        x = self.concat_projection_conv1(x) # 128
        ...
        # graph_fea = spine_graph.detach().to('cpu').numpy() # (n, c, h, w)
        # main_fea = x.detach().to('cpu').numpy() # (n, c, h, w)
        # for c in range(graph_fea.shape[1]):
        #     plt.subplot(121)
        #     plt.imshow(graph_fea[0, c, :, :])
        #     plt.title('graph fea: ' + str(c))
        #
        #     plt.subplot(122)
        #     plt.imshow(main_fea[0, c, :, :])
        #     plt.title('main fea: ' + str(c))
        #     plt.show()

        x = x + spine_graph

        x = F.interpolate(x, size=low_level_features.size()[2:], mode='bilinear', align_corners=True) # stride = 4
        low_level_features = self.feature_projection_conv1(low_level_features)  # 24
        low_level_features = self.feature_projection_bn1(low_level_features)
        low_level_features = self.relu(low_level_features)
        x = torch.cat((x, low_level_features), dim=1) # 152
        x = self.decoder(x)
        x = self.semantic(x)
        logit = F.interpolate(x, size=input.size()[2:], mode='bilinear', align_corners=True)

        return logit, fea_logit
    ...
    def train_fixbn(self, mode=True, freeze_bn=True, freeze_bn_affine=False):
        ...
        Returns:
            Module: self
        """
        super(DeepLabv3_plus_gcn, self).train(mode)
        if freeze_bn:
...
#     image = torch.randn(8, 1, 512, 512) * 255
#     image = image.cuda()
#     model = DeepLabv3_plus_gcn(nInputChannels=1, n_classes=11, os=16, pretrained=False, _print=True)
#     model = model.cuda()
#     # print(model)
