...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta', type=float, nargs='+',
                        help="a list of localization loss weights, [40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0]")
    ...
    if args.ensemble_type == 'model-selection':
        for beta_ind, beta in enumerate(args.beta):
            ...
            for i, case_ind in enumerate(test_ind):
                ...
                pixelspacing = pixelspacing[None, :]

                dist = cal_distances(pred_coords, pred_seg_coords, pixel_spacings=pixelspacing)
                dist = np.mean(dist)
                if dist < min_dists[i]:
                    min_dists[i] = dist
        ...
        for i, case_ind in enumerate(test_ind):
            for beta_ind, beta in enumerate(args.beta):
                ...
                logit = F.interpolate(logit, size=original_img_size, mode='bilinear', align_corners=True)
                prob = F.softmax(logit, dim=1)
                seg = torch.argmax(prob, dim=1).detach()  # (1, h, w)
                if beta_ind == 0:
        ...
        for i, case_ind in enumerate(test_ind):
            ...
            for beta_ind, beta in enumerate(args.beta):
                ...
                # for model-selection
                ms_seg = torch.argmax(prob, dim=1).squeeze(dim=0).detach().to('cpu').numpy()  # (h, w)
                pred_seg_coords = cal_keypoints(ms_seg, class_num=logit.shape[1])
                # ms_seg = ndimage.rotate(ms_seg, angle=-90)
            ...
            segNii = nib.Nifti1Image(seg.astype(np.int16), affine=nii.affine)
            nib.save(segNii, os.path.join(out_dir, 'seg_' + str(case_ind) + '.nii.gz'))
            # print('fold%d Case%d %s dice = %.2f' % (fold_ind, case_ind, flag, dice * 100))
