...
class Series(list):
    ...
    def __init__(self, dicom_list: List[DICOM]):
        ...
        # -------------------revised by Shumao Pang------------------------------
        if len(plane_counter) == 3:
            if (plane_counter.most_common(3)[0][1] - plane_counter.most_common(3)[2][1]) < 2:
                self.plane = 'invalid'
                dicom_list = [dicom for dicom in dicom_list if dicom.plane == 'sagittal']
            else:
