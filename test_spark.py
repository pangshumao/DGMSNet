...
from datasets.utils import pad_image_coords_to_square, unpad_square_image_coords, unpad_square_coords
    ...
    for case_ind in test_ind:
        ...
        if args.task == 'localization':
            ...
            pred_coords = resize_point(pred_coords.squeeze().detach().to('cpu').numpy(), in_img_size=img_size,
                                       out_img_size=original_square_img_size) # square padding space
            pred_coords = unpad_square_coords(pred_coords, original_img_size) # original space
            pred_coords_list.append(pred_coords)
            pred_heatmaps = F.interpolate(pred_heatmaps, size=original_square_img_size, mode='bilinear', align_corners=True)
            pred_heatmaps = pred_heatmaps.squeeze().detach().to('cpu').numpy()
            pred_heatmaps = [unpad_square_image_coords(pred_heatmaps[ind, :, :], original_img_size) for ind in pred_heatmaps.shape[0]]
            pred_heatmaps = np.stack(pred_heatmaps, axis=0)
